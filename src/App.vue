...
const audio = new Audio('/kartonyono_medot_janji.mp3');
audio.loop = true; // Agar lagu diputar berulang

onMounted(() => {
  // Menambahkan event listener untuk memainkan audio setelah interaksi user pertama
  const playAudio = () => {
    audio.play();
...
<template>
  <section class="h-screen w-screen flex items-center justify-center bg-gradient-to-br from-white to-blue-100 overflow-hidden relative">
    ...
    <div class="absolute z-10 -left-80 size-[64rem] rounded-full bg-blue-100/80 blur-3xl"></div>
  </section>
</template>
